...
@server.tool(
    ...
    description=(
        ...
        "if there is none. The query is literal, so punctuation and slashes mean "
        "themselves; regex=true opts into pattern matching, and under fulltext the "
        "dialect is the backend's, named in 'tool'. Results carry each RFC's status "
        "and obsolescence, and title results put current RFCs before superseded ones, "
        "so a page cut short at 'limit' keeps the replacement rather than the document "
